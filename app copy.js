...
function actualizarFiltros() {
    actualizarSelect("filtroCarrera", "carrera", "Todas");
    actualizarSelect("filtroMateria", "materia", "Todas");
    actualizarSelect("filtroAnio", "anio", "Todos");
    actualizarSelect("filtroCuatrimestre", "cuatrimestre", "Todos");
    actualizarSelect("filtroComision", "comision", "Todas");
    actualizarSelect("filtroTrabajo", "trabajo", "Todos");
}
...
function actualizarSelect(id, propiedad, textoTodos) {
    ...
    const nombreFiltro = obtenerNombreFiltro(propiedad);
    const valorActual = filtros[nombreFiltro];
    // -----------------------------------------------------
    // Obtener proyectos compatibles
    // ignorando ESTE filtro
    // -----------------------------------------------------
    const proyectosCompatibles = obtenerProyectosCompatibles(nombreFiltro);
    // -----------------------------------------------------
...
function obtenerNombreFiltro(propiedad) {
    const mapa = {
        carrera: "carrera",
        materia: "materia",
        anio: "anio",
...
function formatearNombreTrabajo(trabajo) {
    if (!trabajo) {
        return "";
    }
    return trabajo
...
function crearOpcionesSelect(id, valores) {
    const select = document.getElementById(id);
    if (!select) {
        return;
    }
    ...
    // Eliminar duplicados
    // -----------------------------------------------------
    const valoresUnicos = [...new Set(valores.filter((valor) => valor !== null && valor !== undefined && valor !== ""))];
    // -----------------------------------------------------
    // Ordenar
    // -----------------------------------------------------
...
function actualizarEstadoVisualFiltros() {
    const selects = {
        ...
        cuatrimestre: document.getElementById("filtroCuatrimestre"),
        comision: document.getElementById("filtroComision"),
        trabajo: document.getElementById("filtroTrabajo"),
    };

    Object.entries(selects).forEach(([nombre, select]) => {
        if (!select) return;

        select.classList.toggle("filtro-activo", filtros[nombre] !== "");
    });
}
function manejarCambioFiltro(evento) {
    ...
    const mapaFiltros = {
        ...
        filtroComision: "comision",
        filtroTrabajo: "trabajo",
    };
    const propiedad = mapaFiltros[id];
    if (!propiedad) {
        return;
    }
    filtros[propiedad] = evento.target.value;
    actualizarFiltros();
    actualizarEstadoVisualFiltros();
    actualizarBotonFiltros();
    renderizarProyectos();
    ...
    return proyectos.filter((proyecto) => {
        ...
        if (filtros.busqueda) {
            const textoBusqueda = [proyecto.alumno, proyecto.tema, proyecto.trabajo, proyecto.descripcion, proyecto.materia, proyecto.comision]
                ...
                .join(" ")
                .toLowerCase();
            if (!textoBusqueda.includes(filtros.busqueda)) {
                return false;
            }
        }
        return true;
...
function actualizarBotonFiltros() {
    ...
    const cantidadFiltros = [filtros.carrera, filtros.materia, filtros.anio, filtros.cuatrimestre, filtros.comision, filtros.trabajo].filter((valor) => valor !== "").length;
...
function limpiarFiltros() {
    ...
    document.getElementById("filtroComision").value = "";
